...
def horizontal_projection_2col(interchange_pixel, col):
    ...
    if col == 2:
        ...
        print(difference)
        for y in range(len(difference)):
            if difference[y] > 30 * 1.5 and difference[y] < 100:
...
def paragraph(row_pixel, col_pixel, col, n):
    ...
    if col == 1:
        return first_img[row_pixel[n * 2 - 2] : row_pixel[n * 2 - 1], 
                         col_pixel[0] : col_pixel[1] ]
    if col == 2:
        return first_img[row_pixel[n * 2 - 2]-10 : row_pixel[n * 2 - 1]+10, 
                         col_pixel[2] : col_pixel[3] ]
...
if len(col_pixel) == 2:
    ...
    paragraph_snap = paragraph_1(row_pixel, chosen_paragraph)
elif len(col_pixel) == 4:

    row_pixel = horizontal_projection_2col(col_pixel, chosen_col)
